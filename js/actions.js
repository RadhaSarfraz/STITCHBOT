$(document).ready(function(){
    ...
    $('#modify-user-checkout').submit(function(e){
        e.preventDefault();
        var f_name = $(".first_name").val();
        var l_name = $(".last_name").val();
        var mobile = $(".mobile").val();
        ...
        if (f_name == '' || l_name == '' || mobile == '' || address == '' || city == ''){
            ...
            $.ajax({
                ...
                success:function(response){
                    ...
                    var res = response;
                    if(res.hasOwnProperty('success')){
                        $('#modify-user-checkout').append('<div class="alert alert-success">Modified Successfully.</div>');
                        setTimeout(function(){ window.location.href = 'user_delivery_details.php'; }, 1500);
                    }else if(res.hasOwnProperty('error')){
                        $('#modify-user').append('<div class="alert alert-danger">'+res.error+'</div>');
                    }
